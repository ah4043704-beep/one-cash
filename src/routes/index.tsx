...
function validate(fullName: string, phone: string): FormErrors {
  ...
  if (trimmedName.length < 3) errors.fullName = 'الرجاء إدخال الاسم الكامل'
  else if (!NAME_PATTERN.test(trimmedName)) errors.fullName = 'يرجى إدخال الاسم بالعربية فقط'
  if (!PHONE_PATTERN.test(phone)) errors.phone = 'رقم الهاتف غير صحيح (مثال: 7XXXXXXXX)'
  return errors
}
...
function OneCashInvitePage() {
  ...
  function handleReset() { setSuccess(null); setFullName(''); setPhone(''); setErrors({}); setSubmitError(null) }
      ...
      <header className="oc-hero-bg px-5 pt-8 pb-10 sm:px-8 sm:pt-10">
        <div className="relative z-10 mx-auto max-w-md sm:max-w-lg">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3"><div className="text-right"><div className="text-xl font-extrabold text-white sm:text-2xl">ون كاش</div><div className="text-[11px] font-medium text-white/70 sm:text-xs">محفظتك الرقمية</div></div></div>
            <div className="oc-logo-badge flex h-14 w-14 shrink-0 flex-col items-center justify-center rounded-2xl sm:h-16 sm:w-16"><span className="text-[13px] font-black leading-tight text-white sm:text-sm">ون</span><span className="text-[13px] font-black leading-tight text-white sm:text-sm">كاش</span></div>
          </div>
          <div className="oc-rise mt-8 flex justify-center" style={{ animationDelay: '80ms' }}><span className="oc-badge inline-flex items-center gap-1.5 rounded-full px-4 py-1.5 text-[13px] font-bold sm:text-sm"><span aria-hidden="true">✦</span>دعوة خاصة لك<span aria-hidden="true">✦</span></span></div>
          <h1 className="oc-rise mt-5 text-center text-[28px] font-extrabold leading-[1.35] sm:text-4xl" style={{ color: '#fdf6ea', animationDelay: '160ms' }}>ادخل بياناتك واحصل على{' '}<span style={{ color: 'var(--oc-gold)' }}>رابط دعوتك</span> الخاص فورًا</h1>
          <p className="oc-rise mx-auto mt-4 max-w-sm text-center text-[15px] leading-relaxed text-white/75 sm:text-base" style={{ animationDelay: '220ms' }}>أدخل اسمك ورقم هاتفك ليتم إنشاء رابط الدعوة الخاص بك مع رابط تحميل التطبيق مباشرة.</p>
          <div className="oc-pop mt-7 flex justify-center" style={{ animationDelay: '300ms' }}><div className="oc-gold-card w-full max-w-xs rounded-2xl px-6 py-5 text-center sm:max-w-sm"><div className="text-2xl font-black text-[#4a2a06] sm:text-3xl">٥٠٠ ريال يمني</div><div className="mt-1 text-[13px] font-semibold text-[#5c3a10] sm:text-sm">هدية ترحيبية عند فتح الحساب</div></div></div>
        </div>
      </header>
      <div className="oc-stripe-divider" />
      <main className="px-5 py-10 sm:px-8">
        <div ref={formCardRef} key={shakeKey} className={`oc-form-card mx-auto max-w-md rounded-[28px] p-6 sm:max-w-lg sm:p-9 ${shakeKey > 0 && (errors.fullName || errors.phone || submitError)? 'oc-shake' : ''}`}>
          {!success? (
            <>
        ...
        <p className="mx-auto mt-9 max-w-md text-center text-[13px] text-[#a8897b] sm:text-sm">ون كاش © 2026 — العرض محدود حتى 30 سبتمبر</p>
      </main>
    </div>
  )
}
